                ...
                <div class="card-header pb-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex align-items-center">
                            <a class="btn btn-success m-0" href="{{ route('courses.create') }}">Thêm</a>
                            &emsp;
                            <form>
                        ...
                        <table class="table align-items-center " id="example">
                            ...
                            @foreach($data as $values)
                                <tr>
                                    ...
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0">{{ $values->name_course }}</p>
                                    </td>
                                    <td class="align-middle text-center">
                                        <span class="text-secondary text-xs font-weight-bold">{{ $values->money_format }}</span>
                                    {{-- <span class="text-secondary text-xs font-weight-bold">{{ $values->year_create_at }}</span>--}}
                                    </td>
                                    <td class="align-middle text-center text-sm">
                                        <span class="badge badge-sm bg-gradient-{{ $values->status_course == 0 ? 'success' : 'warning' }}">
                                            {{ $values->status_course == 0 ? 'Kích hoạt' : 'Chưa kích hoạt' }}
                                        </span>
                                    </td>
                                    <td class="align-middle text-center d-flex justify-content-center align-items-center">
                                        <span class="text-secondary text-xs font-weight-bold"><a class="btn btn-primary m-0" href="{{ route('courses.show', $values) }}">Chi tiết</a></span>&emsp;
                                        <span class="text-secondary text-xs font-weight-bold"><a class="btn btn-secondary m-0" href="{{ route('courses.edit', $values) }}">Sửa</a></span>&emsp;
